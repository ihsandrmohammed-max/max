...
class Transaction:
    ...
    def old_import_data(self, object, **kw):
    ...
    def old_import_data(self, object, data_list=False, **kw):
        msg = "Current channel doesn't allow it."
        _logger.info(">> kw >> : %r", kw)
        _logger.info(">> object >> : %r", object)
    ...
    def import_data(self, object, data_list=False, **kw):
        msg = "Current channel doesn't allow it."
        _logger.info("🚀 import_data() started for object: %r", object)
        _logger.info("⚙️ kw initial: %r", kw)

        success_ids, error_ids, create_ids, update_ids = [], [], [], []
        kw.update(page_size=self.instance.api_record_limit)

        page = 0
        total_record_limit = self.instance.total_record_limit or 0  # ✅ استخدم القيمة الفعلية من الإعدادات
        has_more = True

        if hasattr(self.instance, f'import_{self.channel}'):
            msg = ''
            try:
                while has_more:
                    page += 1
                    _logger.info("📄 Starting Page %s ...", page)

                    s_ids, e_ids, feeds = [], [], False
                    if not data_list:
                        data_result = getattr(self.instance, f'import_{self.channel}')(object, **kw)
                        if isinstance(data_result, tuple):
                            data_list, kw = data_result
                        else:
                            data_list = data_result

                    # ✅ تأكد من أن data_list قائمة صالحة
                    data_list = [] if not data_list else data_list
                    if data_list and isinstance(data_list, list):
                        kw['last_id'] = data_list[-1].get('store_id')

                    _logger.info("📦 Retrieved %s records from API", len(data_list))

                    if data_list:
                        objectmapping = self.getFeedObjectDictionary()
                        if object in objectmapping:
                            s_ids, e_ids, feeds = (
                                self.env[objectmapping.get(object)]
                                .with_context(channel_id=self.instance)
                                .with_company(self.instance.company_id.id)
                                ._create_feeds(data_list)
                            )
                        elif object == 'product.attribute':
                            data_list = (
                                data_list.get('create_ids', [])
                                if data_list.get('create_ids')
                                else data_list.get('update_ids', [])
                            )
                            msg = (
                                ...
                                f"All attributes are synced with <span class='font-weight-bold'>{len(data_list)} attribute sets</span></div>"
                                if data_list
                                else "<div class='alert alert-danger' role='alert'>Attributes failed to import.</div>"
                            )
                        else:
                            raise Exception(f'Invalid object type: {object}')
                    elif not data_list and page == 1:
                        _logger.warning("⚠️ No data available for page 1")
                        msg = "No Data Available to Import!"
                        break

                    # ✅ بعد كل صفحة
                    self._cr.commit()
                    _logger.info("💾 Database committed for page %s", page)

                    success_ids.extend(s_ids)
                    error_ids.extend(e_ids)

                    if self.evaluate_feed and feeds:
                        _logger.info("📥 Importing feed items (creating/updating)...")
                        mapping_ids = feeds.with_context(get_mapping_ids=True).import_items()
                        create_ids.extend([m.id for m in mapping_ids.get('create_ids')])
                        update_ids.extend([m.id for m in mapping_ids.get('update_ids')])
                        self._cr.commit()

                    # ✅ تحقق من وجود صفحات إضافية
                    if len(data_list) < kw.get('page_size', 0):
                        _logger.info("✅ No more pages (data_list < page_size). Ending loop.")
                        has_more = False

                    if total_record_limit and page * (
                            kw.get('page_size') or self.instance.api_record_limit) >= total_record_limit:
                        _logger.info("⛔️ Reached total_record_limit (%s). Stopping import.", total_record_limit)
                        has_more = False

                    # Reset data_list for next page
                    data_list = False

            except Exception as e:
                msg = f'Something went wrong: {e}'
                _logger.exception("❌ Exception in import_data: %s", e)
                msg = (
                    f"<div class='alert alert-danger' role='alert'>"
                    f"<h3 class='alert-heading'><i class='fa fa-frown-o'/> Exception !</h3>"
                    f"<hr><span class='badge badge-pill badge-danger'> {msg} </span></div>"
                )

            _logger.info(
                "🏁 Finished import_data | Total Success: %s | Errors: %s | Created: %s | Updated: %s",
                len(success_ids), len(error_ids), len(create_ids), len(update_ids)
            )

            if not msg:
                operation = dict(IMPORT_OPR)[object]
                msg = self.getActionMessage(
                    msg, success_ids, error_ids, create_ids, update_ids,
                    kw.get('last_id'), self.instance.debug, operation, kw.get('ext_msg')
                )
            if not msg:
                msg = (
                    f"<div class='alert alert-danger' role='alert'>"
                    f"<h3 class='alert-heading'><i class='fa fa-frown-o'/>  Sorry !</h3>"
                    f"<hr><span class='badge badge-pill badge-danger'> 404 </span>"
                    f"<span class='font-weight-bold'> No records found for applied filter.</span></div>"
                )
    ...
    def import_data_webhook(self, object, data_list=False, **kw):
        msg = "Current channel doesn't allow it."
        _logger.info(">> kw >> : %r", kw)
        _logger.info(">> object >> : %r", object)
        ...
        if hasattr(self.instance, 'import_{}'.format(self.channel)):
            ...
            try:
                ...
                if data_list:
                    objectmapping = self.getFeedObjectDictionary()
                    if object in objectmapping:
                        self.env[objectmapping.get(object)].with_context(
                            channel_id=self.instance
                        ).with_company(self.instance.company_id.id)._create_feeds(data_list)
                    elif object == 'product.attribute':
    ...
    def import_data_from_list(self, object, data_list):
        success_ids, error_ids, create_ids, update_ids = [], [], [], []

        if not data_list:
            return self.display_message("No data available")

        objectmapping = self.getFeedObjectDictionary()
        if object not in objectmapping:
            return self.display_message("Invalid object type")

        s_ids, e_ids, feeds = self.env[objectmapping[object]].with_context(
            channel_id=self.instance
        ).with_company(self.instance.company_id.id)._create_feeds(data_list)
        ...
        self._cr.commit()
        return self.display_message("Data imported successfully.")
