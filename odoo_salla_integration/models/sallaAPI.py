...
class SallaApi:
    ...
    # Get Orders
    def old_get_orders(self, **kw):
        _logger.info("get_orders :::: kw : %s", kw)
        order_data_list = []
        ...
        if kw.get('filter_type') == "id":
            ...
            for order_store_id in order_store_ids:
                if order_store_id:
                    ...
                    response = self.salla_response(endpoint, params=params)
                    _logger.info("get_orders :::: response : %s", response)
                    if response:
        ...
        if kw.get('filter_type') == "id":
            ...
            for order_store_id in order_store_ids:
                if order_store_id:
                    ...
                    params_items = {'order_id': order_store_id}
                    items_endpoint = self.import_url + "orders/items/"
                    items_response = self.salla_response(items_endpoint, params=params_items)
                    _logger.info("get_orders :::: items_response : %s", items_response)
                    if items_response:
