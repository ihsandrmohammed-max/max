...
class ProductFeed(models.Model):
    ...
    def old_create_feed(self, product_data):

        variant_data_list = product_data.pop('variants', [])
        feed_id = self._context.get('product_feeds').get(product_data.get('channel_id'), {}).get(
            str(product_data.get('store_id')))
        try:
            if variant_data_list:
                product_data['feed_variants'] = [(0, 0, variant_data) for variant_data in variant_data_list if
                                                 variant_data]
    ...
    def _create_feed(self, product_data):
        if not isinstance(product_data, dict):
            _logger.error(
                "Invalid product_data type in _create_feed: expected dict but got %s. Value: %s",
                type(product_data).__name__,
                product_data,
            )
            return False
        ...
        feed_id = (
            self._context.get('product_feeds', {})
            .get(product_data.get('channel_id'), {})
            .get(str(product_data.get('store_id')))
        )

        try:
            if variant_data_list and isinstance(variant_data_list, (list, tuple)):
                product_data['feed_variants'] = [
                    (0, 0, variant_data)
                    for variant_data in variant_data_list
                    if isinstance(variant_data, dict) and variant_data
                ]
                product_data['feed_variants'].insert(0, (5, 0))
            ...
            _logger.error(
                "Failed to create feed for Product %s (store_id=%s): %s",
            ...
            if not product_exists_odoo:
                ...
                if attribute_value_ids:
                    ...
                    search_product = self.env['product.product'].search(
                        [('product_tmpl_id', '=', template_id.id), ('combination_indices', '=', combination)])
                    if search_product and product_exists_odoo != search_product:
                        raise Exception(
                            f"Product combination already exists in odoo for template '{template_id.name} [ID: {template_id.id}]'<br/>  Product Exists: {search_product.display_name} [ID: {search_product.id}]<br/> Feed Variant: [{variant.default_code}] {variant.name_value}")
    ...
    def wk_change_product_qty(self, product_id, qty_available, location_id):
        #   TODO::  Stop Update Product Quantity
        pass
    ...
    def import_product(self, channel_id):
        ...
        vals.pop('weight_unit', None)

        dimensions_unit = vals.pop('dimensions_unit', None)
        if dimensions_unit:
            vals['dimensions_uom_id'] = channel_id.get_uom_id(name=dimensions_unit).id

        if not vals.pop('wk_product_id_type', None):
            vals['wk_product_id_type'] = 'wk_upc'
        ...
        qty_available = vals.pop('qty_available', 0)
        list_price = vals.get('list_price')
        list_price = list_price and parse_float(list_price) or 0

        image_url = vals.pop('image_url', None)
        if not vals.get('barcode'):
            vals['barcode'] = False
        if not vals.get('default_code'):
            vals['default_code'] = False

        b64_image = vals.pop('image', None)
        if b64_image:
            vals['image_1920'] = b64_image
        elif image_url and (image_url not in ['false', 'False', False]):
            image_res = channel_id.read_website_image_url(image_url)
            if image_res:
                vals['image_1920'] = image_res

        match = self._context.get('product_mappings', {}).get(channel_id.id, {}).get(store_id)
        template_exists_odoo = channel_id.match_odoo_template(vals, variant_lines=feed_variants)
        ...
        if state == 'done':
            if match:
                ...
                extra_categ_ids = vals.pop('extra_categ_ids', None)
                ...
                if extra_categ_ids:
                    ...
                    if categ_ids:
                        ...
                        try:
                            ...
                            if 'product_category_id' in str(e) or 'ForeignKeyViolation' in str(e):
                                _logger.warning(
                                    "⚠️ Skipping missing category for product %s (Error: %s)", self.name, e
                                )
                                self.env.cr.rollback()
                                state = 'error'
                                message += f"<br/>Skipped missing category for {vals.get('name', 'Unknown Product')}"
                            else:
                                raise
                ...
                if len(template_id.product_variant_ids) != (len(variant_lines) or 1):
                    message += f"<br/>⚠️ Variant count mismatch between channel ({len(variant_lines)}) and Odoo ({len(template_id.product_variant_ids)})"

                if len(variant_lines):
                    context['wk_qty_update'] = False
                    res = self.with_context(context).update_product_variants(
                        variant_lines, template_id, store_id, channel_id.location_id, channel_id)
                    if res:
                        message += res
                        state = 'error'
                else:
                    for variant_id in template_id.product_variant_ids:
                        ...
                        match_product = self._context.get('variant_mappings', {}).get(channel_id.id, {}).get(store_id,
                                                                                                             {}).get(
                            'No Variants')
                ...
                try:
                    if not template_exists_odoo:
                        ...
                        if extra_categ_ids:
                            ...
                            if categ_ids:
                                ...
                                try:
                                    ...
                                    _logger.warning("⚠️ Skipped product %s due to missing category. Error: %s",
                                                    vals.get('name'), e)
                                    self.env.cr.rollback()
                                    message += f"<br/>Skipped product {vals.get('name')} due to missing category"
                                    state = 'error'
                            else:
                                state = 'error'
                        else:
                        ...
                        if extra_categ_ids:
                            for cat_id in extra_categ_ids:
                                # ✅ Ensure the ID is a valid integer
                                try:
                                    cat_id_int = int(str(cat_id).strip())
                                except (ValueError, TypeError):
                                    _logger.warning(
                                        f"⚠️ Invalid category ID '{cat_id}' for product {vals.get('name')}, skipping..."
                                    )
                                    continue

                                # ✅ Check if category exists
                                if self.env['product.category'].sudo().search_count([('id', '=', cat_id_int)]) > 0:
                                    valid_extra_cats.append(cat_id_int)
                                else:
                                    # ⚠️ If category is missing, use the first available one
                                    fallback_cat = self.env['product.category'].sudo().search([], limit=1)
                                    if fallback_cat:
                                        valid_extra_cats.append(fallback_cat.id)
                                        _logger.warning(
                                            f"⚠️ Category ID {cat_id_int} not found for product {vals.get('name')} — replaced with category ID {fallback_cat.id} ({fallback_cat.name})"
                                        )
                                    else:
                                        _logger.error("❌ No fallback category found in the system!")

                        # ✅ Use the verified (or replaced) category IDs
                        if valid_extra_cats:
                            res = self.get_extra_categ_ids(valid_extra_cats, channel_id)
                            message += res.get('message', '')
                            categ_ids = res.get('categ_ids')
                            ...
                            _logger.warning(f"⚠️ No valid or fallback extra categories for product {vals.get('name')}")
                    ...
                    if len(variant_lines) and template_id:
                        ...
                        for variant_id in template_id.product_variant_ids:
                            ...
                            if qty_available and eval(str(qty_available)) > 0 and not template_exists_odoo:
                                self.wk_change_product_qty(
                                    variant_id, qty_available, channel_id.location_id)
                            match = self._context.get('variant_mappings', {}).get(channel_id.id, {}).get(store_id,
                                                                                                         {}).get(
                                'No Variants')
                            if not match:
                                channel_id.create_product_mapping(
            ...
            if state == 'done':
                message += f"<br/>✅ Product {vals.get('name', '')} successfully evaluated"
    ...
    def reset_extra_categ_ids(self):
        """
        Reset extra_categ_ids field to empty for selected records.
        This method is called from server action to reset extra_categ_ids
        for multiple selected product feed records.
        """
        if not self:
            return {
        ...
        # Reset extra_categ_ids to False (empty) for all selected records
        count = len(self)
        self.write({'extra_categ_ids': False})
        ...
        return {
            ...
            'params': {
                'title': 'Success',
                'message': f'Extra categories reset for {count} record(s).',
                'type': 'success',
                'sticky': False,
            }
        }
