...
class WkFeed(models.Model):
    ...
    @api.model
    def old_create_feeds(self, data_list):
    ...
    def _create_feeds(self, data_list):
        ...
        _logger.info("🚀 Starting _create_feeds for model: %s | Total items: %d", self._name, len(data_list))

        for idx, data in enumerate(data_list, start=1):
            _logger.info("🔹 Processing item %d/%d | Data type: %s | Value: %s", idx, len(data_list),
                         type(data).__name__, data)

            if not isinstance(data, dict):
                _logger.error(
                    "❌ Invalid data type at index %d: expected dict but got %s. Value: %s",
                    idx, type(data).__name__, data
                )
                error_ids.append(str(data))
                continue
            ...
            try:
                ...
                if feed:
                    self += feed
                    success_ids.append(data.get('store_id'))
                    _logger.info("✅ Feed created successfully for store_id=%s | Feed ID=%s", data.get('store_id'),
                                 feed.id)
                else:
                    error_ids.append(data.get('store_id'))
                    _logger.warning("⚠️ Feed not created for store_id=%s | Data: %s", data.get('store_id'), data)

            except Exception as e:
                _logger.exception(
                    "💥 Exception while creating feed for store_id=%s | Data: %s | Error: %s",
                    data.get('store_id') if isinstance(data, dict) else str(data),
                    data, str(e)
                )
                error_ids.append(data.get('store_id') if isinstance(data, dict) else str(data))

        _logger.info("🏁 Finished _create_feeds | Success: %d | Errors: %d", len(success_ids), len(error_ids))
        if error_ids:
            _logger.warning("⚠️ Error IDs: %s", error_ids)
    ...
    def get_product_id(self, store_product_id, line_variant_ids, channel_id, default_code=None, barcode=None):
        ...
        product_id = None
        _logger.info(self._context.get('variant_mappings'))
        va = self._context.get('variant_mappings')
