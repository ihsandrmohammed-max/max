...
class MultiChannelSale(models.Model):
    ...
    webhook_sales_order = fields.Selection(
        [
            ('disabled', 'Disabled'),
            ('active_api', 'Active API'),
            ('active_webhook', 'Active Webhook'),
        ],
        string='Webhook Sales Order',
        default='disabled'
    )

    webhook_product = fields.Selection(
        [
            ('disabled', 'Disabled'),
            ('active_api', 'Active API'),
            ('active_webhook', 'Active Webhook'),
        ],
        string='Webhook Product',
        default='disabled'
    )

    webhook_category = fields.Selection(
        [
            ('disabled', 'Disabled'),
            ('active_api', 'Active API'),
            ('active_webhook', 'Active Webhook'),
        ],
        string='Webhook Category',
        default='disabled'
    )

    webhook_customer = fields.Selection(
        [
            ('disabled', 'Disabled'),
            ('active_api', 'Active API'),
            ('active_webhook', 'Active Webhook'),
        ],
        string='Webhook Customer',
        default='disabled'
    )

    cash_delivery_product_id = fields.Many2one('product.product', string='Cash on Delivery Product')
    ...
    def set_info_urls(self):
        for instance in self:
            ...
            instance.write({
                'blog_url': url_info.get('blog', '#'),
                'store_url': url_info.get('store', '#')
            })
    ...
    @api.model
    def old_om_format_date(self, date_string):
    ...
    def om_format_date(self, date_string):
        timezone = self.wk_time_zone or "Asia/Riyadh"
        om_date_time = None
        message = ''
        _logger.info("⏱️ [om_format_date] Start processing date_string=%r | wk_time_zone=%r", date_string, timezone)
        try:
            if date_string:
                dt = parser.parse(date_string)
                _logger.debug("📦 Parsed datetime (raw) = %s | tzinfo=%s", dt, dt.tzinfo)
                if dt.tzinfo is None:
                    dt = pytz.timezone(timezone).localize(dt)
                    _logger.debug("🌍 Localized datetime to %s = %s", timezone, dt)
                om_date_time = dt.astimezone(pytz.UTC).replace(tzinfo=None)
                _logger.debug("✅ Converted to UTC = %s", om_date_time)

                # Subtract 3 hours
                # om_date_time = om_date_time - timedelta(hours=3)
                _logger.debug("⏰ After subtracting 3 hours = %s", om_date_time)
            else:
                _logger.warning("⚠️ [om_format_date] Received empty date_string.")
        except Exception as e:
            message = str(e)
            _logger.error("❌ [om_format_date] Error while processing date_string=%r | Error=%s", date_string, message,
                          exc_info=True)
        _logger.info("🏁 [om_format_date] Finished | Output om_date_time=%r | message=%r", om_date_time, message)
        return dict(message=message, om_date_time=om_date_time)
