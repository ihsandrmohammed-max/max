    ...
    def create_order_invoice(self, order_obj):
        ...
        try:
            if not order_obj.invoice_ids:
                ...
                _logger.info(
                    "# ✅ Updated Invoice Info 01: %s",
                    invoice_id.read(['name', 'salla_so_id', 'invoice_date'])
                )

                real_date = order_obj.date_order + timedelta(hours=3)
                _logger.info("# ✅ Updated Invoice Info 02: %s", order_obj.date_order)
                _logger.info("# ✅ Updated Invoice Info 03: %s", real_date)

                invoice_id.write({
                    'salla_so_id': order_obj.origin,
                    'invoice_date': real_date,
                })

                _logger.info(
                    "# ✅ Updated Invoice Info 04: %s",
                    invoice_id.read(['name', 'salla_so_id', 'invoice_date'])
                )
    ...
    def SetOrderPaid(self, payment_data):
        ...
        if not sale_obj.invoice_ids:
            ...
            for invoice in sale_obj.invoice_ids:
                salla_so_id = invoice.salla_so_id
        ...
        if counter <= 1:
            try:
                ...
                real_date = date_invoice + timedelta(hours=3)
                register_wizard_obj = register_wizard.create({
                    'journal_id': journal_id,
                    'payment_date': real_date,

                })
                payment = register_wizard_obj.action_create_payments()
        ...
        for x in sale_obj.invoice_ids:
            _logger.info("Set Memo %s", x)
            _logger.info("Set Memo payment_ids %s", x.payment_ids)
            for y in x.matched_payment_ids:
                y.write({'memo': f"{salla_so_id}"})
                y.move_id.write({'ref': salla_so_id})
                ...
                for l in y.move_id.line_ids.filtered(lambda x: x.account_id.id == default_account_id):
                    l.write({'name': salla_so_id})
                # # For update line payment
                # for l in y.move_id.invoice_line_ids:
                #     l.write({'name': salla_so_id})
            ...
            #
            #
            #     _logger.info("Set Memo %s", y.memo)
            # for line in x.line_ids:
            #     line.write({'name': salla_so_id})
        return {
            'status_message': status_message,
            'status': status
        }
